...
const blankProvider = () => ({
  ...
  auth_header: "x-apisports-key", api_key: "", active: true, priority: 100,
});
      ...
      <div class="card divide-y divide-ink-100 dark:divide-ink-800">
        <div v-for="p in providers" :key="p.id" class="flex flex-wrap items-center justify-between gap-3 p-4">
          <div>
            ...
            <p class="text-xs text-ink-400">{{ p.provider_type }} · priority {{ p.priority }} · {{ p.base_url }} · key {{ p.has_key ? p.api_key_masked : "—" }}</p>
          </div>
